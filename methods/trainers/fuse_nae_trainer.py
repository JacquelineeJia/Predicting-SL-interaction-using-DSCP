...
class FuseNAETrainer(TFTrainer):
  model_class = FuseNAE
  ...
  def __init__(self,dataset):
    super(FuseNAETrainer,self).__init__(dataset)
  ...
  def set_trial(self,trial):
    super(FuseNAETrainer,self).set_trial(trial)

    self.hparam['rank'] = trial.suggest_int('rank',1,512) 
  ...
  def load_data(self,split):
    super(FuseNAETrainer,self).load_data(split)
