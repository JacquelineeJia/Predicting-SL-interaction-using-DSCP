'''
A model similar to DeepSynergy except that it uses molecular graph data for the drugs instead of physicochemical properties
'''
...
# Extracts features from a drug's molecular graph
...
# Generates a cell line embedding
# we use an identity matrix here because there is no eveidence that id matrices are worse then gene expression
